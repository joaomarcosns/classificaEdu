			...
			<div class="two-col-item right">
				...
				<div class="rounded border p-4 bg-slate-50 page-break-inside-avoid">
					...
					<div>
						...
						<div class="font-semibold text-slate-700">{{ $classification?->evaluation_period ?? 'N/A' }}</div>
					</div>
				</div>
			</div>
		</div>
				...
				<table class="table">
					...
					<tbody>
						@foreach ($grades as $grade)
							<tr>
								<td>{{ $grade->evaluation_date?->format('d/m/Y') ?? 'N/A' }}</td>
								<td>{{ trans("grades.periods.{$grade->evaluation_period}") }}</td>
